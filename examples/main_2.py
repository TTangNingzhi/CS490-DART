...
from torchfm.dataset.movielens_2 import MovieLens100KDataset, MovieLens1MDataset, MovieLens20MDataset
from torchfm.dataset.custom import AdressaDataset, AmazonBookDataset, YelpDataset, BookCrossingDataset, JesterDataset
...
def get_dataset(name, path):
    if name == 'movielens100K':
        ...
        return AdressaDataset(path)
    elif name == 'amazon-book':
        return AmazonBookDataset(path)
    elif name == 'yelp':
        return YelpDataset(path)
    elif name == 'book-crossing':
        return BookCrossingDataset(path)
    elif name == 'jester':
        return JesterDataset(path)
    else:
...
def train(model, optimizer, data_loader, criterion, device, log_interval=100, epoch_i=0, method="none"):
    assert method in ["none", "discard", "relabel", "mixed",
                      "reweight", "bootstrap_hard", "bootstrap_soft", 'lcd', 'lcd-re']
    model.train()
    total_loss = 0
    ...
    for i, (fields, target, mask) in enumerate(tk0):
        ...
        if method == "none":
        ...
        elif method in ["discard", "relabel", "mixed"]:
            Ti, Tm, taum, r = 0, 20000, 0.04, 0
            handle_rate = max(min(taum, taum / Tm * (num_iter - Ti)), 0)
            ...
            if method == "discard":
                ...
                discard_mask = torch.zeros_like(target, dtype=torch.bool)
                discard_mask[high_loss_index[int(high_loss_num * r):]] = True
                discard_mask[target == 0] = False
                target_mixed = target.clone()
                target_mixed[high_loss_index] = 0
                loss = criterion(y[~discard_mask], target_mixed[~discard_mask].float())
    ...
    if method in ['discard', 'relabel', 'mixed'] and num_noise > 0 and num_handle > 0:
...
def main(dataset_name,
    ...
    # dataset.clean_test(test_indices=train_dataset.indices)
    dataset.clean_test(test_indices=valid_dataset.indices)
    dataset.clean_test(test_indices=test_dataset.indices)
    ...
    for epoch_i in range(epoch):
        ...
        print('epoch:', epoch_i, 'testing: auc:', test(model, test_data_loader, device))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', default='book-crossing', help='criteo, avazu, movielens100K, movielens1M,'
                                                                        ' or adressa, amazon-book, yelp, book-crossing,'
                                                                        ' jester')
    ...
    parser.add_argument('--method', type=str, default='mixed', help='none, mixed, relabel, discard, reweight, lcd, '
                                                                    'lcd-re, bootstrap_soft, or bootstrap_hard')
    args = parser.parse_args()

    if args.dataset_name == 'criteo':
        dataset_path = 'data/criteo/train.txt'
    elif args.dataset_name == 'avazu':
        dataset_path = 'data/avazu/train'
    elif args.dataset_name == 'movielens100K':
        dataset_path = 'data/ml-100k/u.data'
    elif args.dataset_name == 'movielens1M':
        dataset_path = 'data/ml-1m/ratings.dat'
    elif args.dataset_name == 'adressa':
        dataset_path = 'data/adressa/adressa.dat'
    elif args.dataset_name == 'amazon-book':
        dataset_path = 'data/amazon_book/amazon_book.dat'
    elif args.dataset_name == 'yelp':
        dataset_path = 'data/yelp/yelp.dat'
    elif args.dataset_name == 'book-crossing':
        dataset_path = 'data/book_crossing/book_crossing.dat'
    elif args.dataset_name == 'jester':
        dataset_path = 'data/Jester/jester.dat'
    else:
