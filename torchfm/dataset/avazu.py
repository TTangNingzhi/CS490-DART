...
class AvazuDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path=None, cache_path='.avazu', rebuild_cache=False, min_threshold=4):
        ...
        self.noise_mask = np.zeros(self.length, dtype=np.bool)

    def __getitem__(self, index):
        with self.env.begin(write=False) as txn:
            np_array = np.frombuffer(
                txn.get(struct.pack('>I', index)), dtype=np.uint32).astype(dtype=np.long)
        if np_array[0] == 0 and self.noise_mask[index]:
            np_array[0] = 1
        elif np_array[0] == 1 and self.noise_mask[index]:
            self.noise_mask[index] = False
        return np_array[1:], np_array[0], self.noise_mask[index]
    ...
    def noisify(self, train_indices, noise_rate):
        noise_indices = np.random.choice(train_indices, size=int(noise_rate * len(train_indices)),
                                         replace=False)
        self.noise_mask[noise_indices] = True
        print("Dataset size: {}".format(self.length), "Positive ratio: 10.75%")
        print("# of noise samples: {}".format(np.sum(self.noise_mask)))

    def relabel(self, handle_indices):
        self.noise_mask[handle_indices] = False
